...
- Closed since the prior pass: [reconnect-backoff-never-resets](docs/backlog/closed/bug-2026-06-10-reconnect-backoff-never-resets.md) (was Now #1 / dispatch-races #1, bug medium), resolved 2026-06-20. `connect()`/`session()` now signal an established session via a `registered`/`listened` bool, and both Run loops reset backoff to 1s before sleeping after a healthy drop, so the first reconnect is prompt instead of pinned at the accumulated 60s cap; exponential backoff is preserved for repeated unhealthy failures. A first reset-after-sleep implementation was caught in verification and corrected to reset-before-sleep.
- No new backlog items filed this cycle (verify's low findings were reviewer-rated no-action).
- Reordered: with reconnect-backoff-never-resets shipped, Now backfills - `sweeper-wedges-dirty-delete`, `agent-proctree-windows-race`, and `agent-token-windows-acl` lead; `inconsistent-state-window-reenroll` and `spa-pagination-cursor-corruption` fill Next; Later surfaces `cli-logs-terminal-race-hang` and `web-job-status-vocabulary-drift`. The dispatch-races theme drops to 2 items.
