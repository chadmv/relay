...
- Closed since the prior pass: [sweeper-wedges-dirty-delete](docs/backlog/closed/bug-2026-06-10-sweeper-wedges-dirty-delete.md) (was Now #1 / agent-perforce #1, bug medium), resolved 2026-06-20. `evict()` now skips `DeleteClient` on a `DirtyDelete` entry (the client is already gone) and `SweepOnce` continues past per-entry evict failures instead of aborting the whole pass, so a single dirty delete no longer permanently wedges the sweeper and disk pressure keeps being relieved.
- New item filed: [evictworkspace-state-leak-toctou](docs/backlog/bug-2026-06-20-evictworkspace-state-leak-toctou.md) (bug, medium) - split from the sweeper-wedge fix during planning; `Provider.EvictWorkspace`'s ad-hoc Sweeper lacks `OnEvictedCB` (per-task state leak) and has a lock TOCTOU against a concurrent `Prepare`.
- Reordered: with sweeper-wedges-dirty-delete shipped, Now backfills - `agent-proctree-windows-race`, `agent-token-windows-acl`, and the new `evictworkspace-state-leak-toctou` lead; `inconsistent-state-window-reenroll` and `spa-pagination-cursor-corruption` fill Next; Later keeps `cli-logs-terminal-race-hang` and `web-job-status-vocabulary-drift`. The agent-perforce theme swaps the closed wedge for the new EvictWorkspace item.
