...
### Now
- [Stale teardown can still clobber during the finishRegister gap](docs/backlog/bug-2026-06-19-finishregister-gap-connection-epoch-race.md) - the residual duplicate-execution race left by the just-shipped identity-checked-teardown fix; a stale connection's teardown can still flip a live reconnected worker offline and requeue tasks it is actively running. Same invariant, narrower window.
...
- Shipped since the 2026-06-18 roadmap (7 items): the entire prior **Now** tier - [requeue-paths-skip-epoch-bump](docs/backlog/closed/bug-2026-06-10-requeue-paths-skip-epoch-bump.md), [stale-stream-teardown-clobbers-registration](docs/backlog/closed/bug-2026-06-10-stale-stream-teardown-clobbers-registration.md), [source-tasks-run-without-workspace](docs/backlog/closed/bug-2026-06-10-source-tasks-run-without-workspace.md), [cron-jobs-drop-source](docs/backlog/closed/bug-2026-06-10-cron-jobs-drop-source.md) - plus three prior **Next** items: [perforce-registry-races](docs/backlog/closed/bug-2026-06-10-perforce-registry-races.md), [agent-pipe-drain-hang](docs/backlog/closed/bug-2026-06-10-agent-pipe-drain-hang.md), and [job-status-recompute-race](docs/backlog/closed/bug-2026-06-10-job-status-recompute-race.md) (the most recent, via PR #36).
- New (5 items, from the 2026-06-19 sessions): [finishregister-gap-connection-epoch-race](docs/backlog/bug-2026-06-19-finishregister-gap-connection-epoch-race.md), [forced-cancel-send-backpressure](docs/backlog/bug-2026-06-19-forced-cancel-send-backpressure.md), [dispatch-provider-capability-filter](docs/backlog/bug-2026-06-19-dispatch-provider-capability-filter.md), [drain-mode-disable-test-asserts-running](docs/backlog/bug-2026-06-19-drain-mode-disable-test-asserts-running.md), and [race-test-target-perforce-package](docs/backlog/idea-2026-06-19-race-test-target-perforce-package.md).
- Reordered: the Now tier is rebuilt from the highest-value open correctness items. `finishregister-gap-connection-epoch-race` is promoted to Now #1 as the direct residual of the just-shipped teardown fix (same invariant, same duplicate-execution failure mode), and `schedrunner-poisoned-tick` rises into Now as the last open scheduler-correctness item.

## Dispatch, epoch & stream-teardown races (dispatch-races)

1. [Stale teardown can still clobber during the finishRegister gap (needs worker connection-epoch fence)](docs/backlog/bug-2026-06-19-finishregister-gap-connection-epoch-race.md) (bug, medium) - `finishRegister` registers ownership last, after writing `status=online` and cancelling grace; a stale teardown can pass the ownership gate before the fresh `Register`, then land its offline/grace writes after, flipping a live worker offline and requeuing running tasks. The gate's check and action are not atomic w.r.t. the fresh registration.
2. [Dispatch failure paths are inconsistent and silent](docs/backlog/bug-2026-06-10-dispatch-failure-paths-silent.md) (bug, medium) - partially mitigated, but the bad-source-JSON path still strands a claimed task and the top-level dispatch loop still swallows DB errors with no log.
3. [Agent send goroutine not awaited across reconnect](docs/backlog/bug-2026-06-18-agent-reconnect-send-goroutine-not-awaited.md) (bug, medium) - `connect()` returns without joining its send goroutine, so a reconnect can leave two goroutines draining one `sendCh` and silently drop a queued message. Violates the one-bounded-sender invariant.
4. [Reconnect backoff never resets in agent and NotifyListener](docs/backlog/bug-2026-06-10-reconnect-backoff-never-resets.md) (bug, medium) - the backoff reset is unreachable on the drop path, so reconnect delay degrades monotonically toward the 60s cap and dispatch latency falls back to the 30s safety poll.
5. [Drain-mode disable test asserts 'running' but seedRunningTask seeds 'dispatched'](docs/backlog/bug-2026-06-19-drain-mode-disable-test-asserts-running.md) (bug, medium) - the test has never passed in this configuration; `seedRunningTask` claims via `ClaimTaskForWorker` (leaving `dispatched`) but the test asserts `running`.
