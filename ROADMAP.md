...
- Closed since the prior pass: [agent-reconnect-send-goroutine-not-awaited](docs/backlog/closed/bug-2026-06-18-agent-reconnect-send-goroutine-not-awaited.md) (was Now #1 / dispatch-races #1, bug medium), resolved 2026-06-20. A `sendWG` WaitGroup now joins the previous agent send goroutine before each `connect()`, so at most one sender reads `sendCh` across a reconnect (restoring the one-bounded-sender invariant). The single in-flight message on the dead connection is a documented bounded loss (one log chunk per drop), not re-enqueued.
- No new backlog items filed this cycle (one low/cosmetic verify finding on already-merged code, rated correct-either-way, was below the filing bar).
- Reordered: with agent-reconnect-send-goroutine-not-awaited shipped, Now backfills - `reconnect-backoff-never-resets`, `sweeper-wedges-dirty-delete`, and `agent-proctree-windows-race` lead; `agent-token-windows-acl` and `inconsistent-state-window-reenroll` fill Next; Later surfaces the next user-facing correctness mediums `spa-pagination-cursor-corruption` and `cli-logs-terminal-race-hang`. The dispatch-races theme drops to 3 items.

## Dispatch, epoch & stream-teardown races (dispatch-races)

1. [Reconnect backoff never resets in agent and NotifyListener](docs/backlog/bug-2026-06-10-reconnect-backoff-never-resets.md) (bug, medium) - the backoff reset is unreachable on the drop path, so reconnect delay degrades monotonically toward the 60s cap and dispatch latency falls back to the 30s safety poll.
2. [Cancel/disable handlers send synchronously to workers](docs/backlog/bug-2026-06-10-cancel-disable-handlers-send-synchronously.md) (bug, low) - bounded after the wedged-agent fix, but a multi-task job on one wedged worker can still take up to N x ~5s in the request path.
3. [Brief inconsistent-state window on worker re-enroll](docs/backlog/bug-2026-06-05-inconsistent-state-window-reenroll.md) (bug, low) - between the enroll tx clearing `revoked_at` and the post-commit status flip, a worker momentarily appears revoked with a null timestamp.
