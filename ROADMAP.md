...
- Closed since the prior pass: [agent-proctree-windows-race](docs/backlog/closed/bug-2026-06-20-agent-proctree-windows-race.md) (was Now #1 / agent-perforce #1, bug medium), resolved 2026-06-20. `setupProcTree` now returns `(assign, cleanup)` and `Run` calls `assign()` synchronously after `cmd.Start()`, so the `cmd.Process` read happens-after the Start write (no poll goroutine) - removing the Windows-only data race. `relay/internal/agent` is re-included in `make test-race`; verified load-bearing-red under `-race` on Windows with the MSYS2 toolchain.
- No new backlog items filed this cycle.
- Reordered: with agent-proctree-windows-race shipped, Now backfills - `agent-token-windows-acl`, `evictworkspace-state-leak-toctou`, and `inconsistent-state-window-reenroll` lead; `spa-pagination-cursor-corruption` and `cli-logs-terminal-race-hang` fill Next; Later surfaces `web-job-status-vocabulary-drift` and `mcp-structured-errors-lost`. The agent-perforce theme drops to 7 items.
