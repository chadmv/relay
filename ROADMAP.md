...
> Generated by /roadmap on 2026-06-21 (refresh). Source of truth is docs/backlog/ - edit
...
### Now
- [Provider.EvictWorkspace leaks per-task state and has a lock TOCTOU](docs/backlog/bug-2026-06-20-evictworkspace-state-leak-toctou.md) - medium; the ad-hoc Sweeper in `EvictWorkspace` has no `OnEvictedCB` (per-task state survives a manual eviction) and a TOCTOU lock gap against a concurrent `Prepare`. Split from the sweeper-wedge fix.
...
- Closed since the prior pass: [agent-token-windows-acl](docs/backlog/closed/bug-2026-06-10-agent-token-windows-acl.md) (was Now #1 / agent-perforce #1, bug medium), resolved 2026-06-21. On Windows the agent token file now gets a PROTECTED (non-inherited) DACL granting full control only to Owner Rights + LocalSystem + Administrators, severing the inherited ProgramData `BUILTIN\Users` ACE that let any local user read the bearer token; `Persist` hard-fails and removes the file on an ACL error. Verified on Windows with a DACL read-back test (protected bit + exact ACEs), proven load-bearing.
- No new backlog items filed this cycle (both review findings - a vacuous hermetic test and an error-path exposure - were fixed in-iteration).
- Reordered: with agent-token-windows-acl shipped, Now backfills - `evictworkspace-state-leak-toctou`, `inconsistent-state-window-reenroll`, and `spa-pagination-cursor-corruption` lead; `cli-logs-terminal-race-hang` and `web-job-status-vocabulary-drift` fill Next; Later surfaces `mcp-structured-errors-lost` and `admin-output-stderr`. The agent-perforce theme drops to 6 items.
...
1. [Provider.EvictWorkspace leaks per-task state and has a lock TOCTOU](docs/backlog/bug-2026-06-20-evictworkspace-state-leak-toctou.md) (bug, medium) - the ad-hoc Sweeper built by `EvictWorkspace` has no `OnEvictedCB`, so `InvalidateWorkspace` never runs on a manual eviction and per-task state survives; its `lockedShortIDs()` check also has a TOCTOU window against a concurrent `Prepare`. Split from sweeper-wedges-dirty-delete.
