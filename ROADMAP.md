...
- Closed since the prior 2026-06-20 pass: [race-test-target-perforce-package](docs/backlog/closed/idea-2026-06-19-race-test-target-perforce-package.md) (was Now #1, idea medium) and [vet-integration-tagged-build](docs/backlog/closed/idea-2026-06-20-vet-integration-tagged-build.md) (was cli-client #5, idea medium, folded in), both resolved 2026-06-20. The project had no Go CI at all, so the work created the first Go CI workflow (`.github/workflows/go-ci.yml`): it runs `make vet-integration` (`go vet -tags integration ./...`, catching integration-tagged compile breaks) then the full `go test -race ./...` on Linux. A `make test-race` target was added (descoped to exclude `internal/agent` for local Windows devs; CI runs the full set on Linux where it is clean).
- New backlog item filed this cycle: [agent-proctree-windows-race](docs/backlog/bug-2026-06-20-agent-proctree-windows-race.md) (agent-perforce, medium) - the first whole-module `-race` run surfaced a pre-existing Windows-only data race in `setupProcTree` (`proctree_windows.go` reads the `*exec.Cmd` concurrently with `cmd.Start()`); Linux compiles the clean build, so CI is unaffected.
- Reordered: with race-test-target shipped, Now backfills - `trailing-slash-breaks-posts`, `spa-query-cache-logout`, and `dispatch-failure-paths-silent` lead; `agent-reconnect-send-goroutine-not-awaited` and `reconnect-backoff-never-resets` fill Next; Later surfaces the newly-filed `agent-proctree-windows-race`. The cli-client theme drops to 4 items; agent-perforce swaps the closed race-test idea for the proctree race bug.
