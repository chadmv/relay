...
- Closed since the prior 2026-06-20 pass: [trailing-slash-breaks-posts](docs/backlog/closed/bug-2026-06-10-trailing-slash-breaks-posts.md) (was Now #1 / cli-client #1, bug medium), resolved 2026-06-20. `relayclient.NewClient` now normalizes the base URL once with `strings.TrimRight(serverURL, "/")`, so a trailing slash in `RELAY_URL` no longer produces `//v1/...` (which the server 301-redirected, downgrading the POST to a GET and 405ing `relay login`). Centralized at the single constructor; covered by a table test plus a behavioral POST-through-trailing-slash test.
- No new backlog items filed this cycle (the one-line fix surfaced nothing).
- Reordered: with trailing-slash shipped, Now backfills - `spa-query-cache-logout`, `dispatch-failure-paths-silent`, and `agent-reconnect-send-goroutine-not-awaited` lead; `reconnect-backoff-never-resets` and `sweeper-wedges-dirty-delete` fill Next; Later surfaces `agent-token-windows-acl` (a Windows agent-token security item). The cli-client theme drops to 3 items.
