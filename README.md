...
**Exclusions need every agent in the fleet on a build that supports them, and nothing checks that.** Protobuf drops a field an agent's build does not know, so an older agent syncs the excluded path in full and reports success: there is no error, no warning, and no log line saying the exclusion was ignored, and the resulting full-size workspace is registered under the same key an excluding workspace would use. The coordinator does not ask an agent whether it honours exclusions and does not detect the mismatch afterwards, so on a mixed-version fleet whether a task's exclusions take effect depends on which agent it lands on. Upgrade every agent before using exclusions.
