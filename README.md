...
| `RELAY_WORKSPACE_CLOBBER` | Whether relay-created workspace clients carry the p4 option `clobber` in place of `noclobber`. Default `false`. When true, `p4 sync` overwrites a writable, unopened file under the workspace root instead of aborting on it. **Relay workspaces are shared across tasks** under `ModeShared` admission, so what this permits is one task's writable output, sitting in a path another task syncs, being destroyed by that sync. It is a deployment-wide decision: there is no per-job override and no job spec can request it. p4 does not clobber a file that is *opened in the client*, so files unshelved into a relay-created pending changelist are unaffected - that is a statement about p4's open state, not about which files matter. The client spec is rewritten on every prepare, so setting this and restarting the agent updates each stream's client on its next prepare; no eviction and no manual `p4 client`. It also survives a repair that the same setting inside a `client_template` does not: `-t` is applied only when the workspace is cold, so a prepare that recreates a deleted client refetches p4 defaults and drops every template field including `Options:`, while this is reapplied after the fetch every time. If the fetched spec has no `Options:` line, or its value is empty or carries a non-alphabetic token, the spec is left unchanged and the agent logs a warning naming the client. |
