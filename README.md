...
- **Nothing here carries an address, a prefix or a hostname**, and nothing ever will - the refusal path is reachable by any unauthenticated peer. The rule is not "every value is a number": it is that every value is a count or a level unless a specific field has been argued for by SHAPE, in the commit that adds it. `started_at` is the one exemption today, admitted as an RFC 3339 instant. A field carrying a caller-supplied byte, or a map whose key cardinality an unauthenticated peer controls, is inadmissible whatever its type.
...
- **Reading `ingest_log_budget`.** Agents can drive log volume, so the five per-message log sites on the gRPC receive path - the five keys below - are rate-limited per connection; these are the lines that limiter dropped, since `started_at`. **The budget covers those five sites and no others.** Registration-time and status-handling lines (auto-enrollment, inventory replace at reconnect, and `handleTaskStatus`'s retry/status-update/dependency-failure errors) are outside it, so a database outage can drive log volume through them while every number here stays at zero. **`deduped`** is the healthy arm: the same failure repeated inside a five-minute window, folded into one line - a large number here next to a quiet log is one task failing over and over, and the number is how many occurrences that single line represents. **`suppressed`** is the loud arm: a line dropped entirely because that connection's token bucket was empty. A non-zero `suppressed` means some connection is producing *distinct* failures faster than six lines per minute, which is either an attack or a misconfiguration; `39984` under `task_log_persist` is the difference between "a flood is in progress" and "the fleet looks quiet". The five keys name the log site, not the worker: **there is no per-worker split and there will not be one**, because keying these counters on anything the recv goroutine would have to look up needs a shared map write on the one path that must stay lock-free.
- **What `ingest_log_budget` does NOT count.** These are *log lines the budget dropped*, not diagnostics lost. A handler that decides not to log without consulting the budget - a status update for a task that no longer exists, a log chunk the fence rejected, or any of the log sites outside the budget listed above - contributes nothing to these numbers. They also have no `levels` half: each budget is per connection and dies with it, so there is no process-wide current figure to report.
