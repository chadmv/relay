...
| `RELAY_JOB_SUBMIT_RATE_LIMIT` | `120:10s` | Per-**user** rate limit (format `N:duration`) over the three routes that let one principal buy task execution: `POST /v1/jobs`, `POST /v1/jobs/{id}/retry` and `POST /v1/scheduled-jobs/{id}/run-now`. **One shared bucket keyed on the authenticated user, not on the source address.** The anti-enumeration reasoning that makes `RemoteAddr` right for the two rows above does not transfer once a principal exists: keyed on the address, one office egress or load balancer collapses a whole studio into one bucket while one user with a workstation and a laptop gets two. A refusal is `429` and carries `Retry-After`, which is correct for a scripted client; note that no first-party client surfaces it, because `relayclient.ResponseError` and the SPA's `ApiError` both carry the status and not the headers. **It bounds repetition of expensive work, not request volume** - the limiter runs inside the auth chain, so every attempt, refused or not, still costs one token lookup; the control for request volume would be an admission bound at the HTTP listener, which relay does not have. **Per replica and in memory**, like the counters section: two replicas give a caller twice the budget and a restart clears every bucket. **No zero count and no zero window is accepted** - `ParseRateLimit` refuses both, and the server refuses to boot. Every other value boots, and the two halves do not move the bound in the same direction: the count is the burst and the window is the recovery time, so **shrinking the window loosens the bound**. `1:1ns` parses and reads as the tightest value the format can express while admitting one request per clock tick. Widen a bound that is too tight by raising the count, which leaves it visible as a number in the environment. Because `retry` shares the budget, a user whose script has spent it cannot retry a real failed job until the window clears: ten seconds at the default. **`POST /v1/scheduled-jobs` is deliberately not in this bucket** - a creation-rate limit bounds how fast `scheduled_jobs` fills, not how full it gets, and `RELAY_MAX_SCHEDULES_PER_OWNER` is what bounds how full it gets - and **no HTTP rate limit anywhere bounds a schedule's own firing**. **The budget is per principal, so this is only as tight as account creation is:** with `RELAY_ALLOW_SELF_REGISTER=true` an actor mints accounts and buys one budget each, and because `POST /v1/auth/register` is keyed on the source address per `/128`, an IPv6 caller holding a `/64` mints them at an effectively unbounded rate. For a farm whose users are not all trusted, `10:1m` is the recommended value; its cost is that a 200-job `for` loop is refused partway and the workflow becomes one job with 200 tasks, which is the model relay is built around anyway. |
| `RELAY_DB_STATEMENT_TIMEOUT` | `30s` | `statement_timeout` applied to every pooled connection, as a Go duration. **Applied in Go, never through the DSN** - `migrateDSN` is derived from the DSN by prefix rewriting, so a DSN-carried value would reach migrations, and a long `CREATE INDEX` is exactly what must not be cut short. Migrations are structurally out of reach as configured here, because `store.Migrate` opens its own connection before the pool is built. **A value below `1ms` is refused at boot**: `statement_timeout` counts milliseconds and `0` means DISABLED in Postgres, so anything under a millisecond would silently unarm the control rather than tighten it. This is a floor under a query with no owner, not a tuning knob - at the measured `?q=` cost it does not bind, and the row below is what bounds that. |
| `RELAY_JOB_SEARCH_RATE_LIMIT` | `120:10s` | Per-**user** rate limit over list requests that carry `?q=`, on `GET /v1/jobs` and `GET /v1/scheduled-jobs`. **One shared bucket**, separate from the submit bucket above, and keyed the same way and for the same reason. **It is charged inside the handler, at the point the needle is known to be present**, so a list request without `?q=` is never charged and a `?q=` of only whitespace - which the parser trims to absent - is not either. **This is a fairness bound, not a CPU budget**: at the measured cost a full window still exceeds one box, and the bound degrades linearly in the number of accounts an actor controls. Note the constraint is relay's own client as much as an adversary - the jobs view refetches five lanes every three seconds, each carrying the needle while one sits in the search box. |
| `RELAY_PASSWORD_CHANGE_RATE_LIMIT` | `5:1m` | Per-**user** rate limit (format `N:duration`) over `PUT /v1/users/me/password`, and that route only. **One bucket keyed on the authenticated user, not on the source address** - the same key and the same reasoning as the submit row above, which is not repeated here. **Why the bound is this small:** the handler runs a bcrypt compare at cost 12 on every request that gets past its eight-character length guard, and a second bcrypt operation on success, so the work is CPU by construction and cannot be made cheaper without lowering the cost factor that protects stored passwords. Measured on the reference machine at about 185 ms for the compare and about 370 ms for a successful change, so a full window at the default is under two CPU-seconds per minute per principal. **What it costs a legitimate user:** somebody who mistypes their current password five times inside a minute waits up to a minute before the sixth attempt. Their session is untouched and they are not locked out. **Why it is tighter than `RELAY_LOGIN_RATE_LIMIT`:** a refused login is a user who cannot get in; a refused password change is a user who already holds a valid session and waits. A refusal is `429` and carries `Retry-After`, which is correct for a scripted client; no first-party client surfaces it, because `relayclient.ResponseError` and the SPA's `ApiError` both carry the status and not the headers. **It bounds repetition of the bcrypt work, not request volume** - the limiter runs inside the auth chain, so every attempt, refused or not, still costs one token lookup; the control for request volume would be an admission bound at the HTTP listener, which relay does not have. **Per replica and in memory:** two replicas give a caller twice the budget and a restart clears every bucket. **No zero count and no zero window is accepted**, and a smaller window is a looser bound rather than a tighter one, exactly as described for `RELAY_JOB_SUBMIT_RATE_LIMIT` above; widen this bound by raising the count, for example `100000:1m`. **The budget is per principal, so what an actor with M accounts buys is M x 5 attempts per minute, each costing one compare and, on success, a second bcrypt operation**; minting accounts through `POST /v1/auth/register` costs a cost-12 hash and is bounded by `RELAY_REGISTER_RATE_LIMIT` per source address, which is a real brake and a weak one because that limiter keys per IPv6 `/128`. For a farm whose users are not all trusted, `3:1m` is the recommended value; its whole cost is that a user who mistypes three times waits up to a minute, because no workflow on this route is scripted. `POST /v1/users/password-reset` and `POST /v1/users` also run a cost-12 hash and are deliberately **not** covered: both are admin-only, and rate-limiting an admin remedy refuses the operator's action during the incident they are responding to. |
...
| `RELAY_SYNC_HEARTBEAT_INTERVAL` | How often a running `p4 sync` writes a progress summary to the task log (`4m30s; 12483 files; 0 other lines; 811 GB free; last "//depot/..."`). Default `30s`. **A zero with a unit (`0s`) disables the timer**; the completion line still carries a summary. A bare `0`, a negative value, a value too large to represent as a duration, or an interval under `5s` is refused with a warning and the default is used. The file count is a snapshot of a stream still in flight, so it is a lower bound on a running sync; it counts the lines p4 printed that parse as a depot path, not the files p4 transferred - the agent counts output lines and does not interpret the action each one reports. The trailing depot path is quoted because it is the one field whose content comes from the depot. |
| `RELAY_WORKSPACE_CLOBBER` | Whether relay-created workspace clients carry the p4 option `clobber` in place of `noclobber`. Default `false`. When true, `p4 sync` overwrites a writable, unopened file under the workspace root instead of aborting on it. **Relay workspaces are shared across tasks** under `ModeShared` admission, so what this permits is one task's writable output, sitting in a path another task syncs, being destroyed by that sync. It is set on the agent and there is no per-job override for it, though `-t` inherits a template's non-View fields, so a `client_template` a job spec names is a separate route to the same p4 option. p4 does not clobber a file that is *opened in the client*, so files unshelved into a relay-created pending changelist are unaffected - that is a statement about p4's open state, not about which files matter. The client spec is rewritten on every prepare, so setting this and restarting the agent updates each stream's client on its next prepare; no eviction and no manual `p4 client`. If the fetched spec has no `Options:` line, or its value is empty or carries a non-alphabetic token, the spec is left unchanged and the agent logs a warning naming the client. |
| `RELAY_WORKSPACE_MAX_EXCLUSION_SETS` | How many **exclusion-derived** workspaces one agent may hold **per stream**. Not a fleet total and not an agent total, and the stream's own no-exclusion workspace is outside the count. Default `4`, hard maximum `64` (a larger value is clamped with a warning). **There is no value that disables it, and `0` is not one**: `0`, a negative value and an unparseable value all resolve to the default, each with a warning naming the variable and saying the value was not used. Note that `RELAY_WORKSPACE_MAX_AGE` and `RELAY_WORKSPACE_MIN_FREE_GB` above *do* mean "disabled" at zero; this one does not. At the limit the agent reclaims the least valuable unheld exclusion-derived workspace for that stream (an unsynced one first, then the least recently used) rather than refusing, and refuses a prepare only when every slot is held by a running task. On a refusal, in order: find which job specs are minting distinct exclusion sets on that stream, since a refusal means every slot is in use; make the fleet's exclusion sets uniform for that stream; and last, raise the number, at the cost of one more nearly-full workspace per step. Note that the check and the workspace creation are not one atomic step, so two cold prepares for two different new exclusion sets on one stream can both be admitted: the effective bound is this number plus that agent's concurrent prepares on that stream, minus one. |
...
| `stream` | Yes | Perforce stream path. relay addresses p4 by client path, so the stream's own view - remaps included - is what defines the layout on disk. The part of a sync path that follows the stream is interpreted in the client's layout, which for a remapped stream is not the depot layout, so a narrower subpath resolves through the remap and may not name what the author intended. Workspaces are keyed by stream, plus the exclusion set when the spec has one (see below), and reused across tasks. The workspace key is bounded at 512 bytes; the composite form (any spec with an exclusion) adds a fixed 20 bytes, so a stream over 492 bytes falls outside the bound as soon as one exclusion is added. A workspace whose key exceeds the bound is not recorded in the coordinator's inventory, so tasks on that stream always dispatch cold and the workspace does not appear under `relay workers workspaces`. The task still runs. |
...
Active workspaces (held by a running task) are never evicted. Admins can also evict on demand via `relay workers evict-workspace`. Workspaces are also reclaimed outside the sweeper, at prepare time, by the per-stream exclusion-set ceiling described above.

**Sync heartbeat.** A running `p4 sync` writes a periodic summary to the task log (`RELAY_SYNC_HEARTBEAT_INTERVAL`), which is how you tell a live multi-hour transfer from a wedged one - the elapsed field moves whether or not p4 has written anything. Its free-space figure is read on the same volume `RELAY_WORKSPACE_MIN_FREE_GB` is compared against, so the two numbers are directly comparable when a sweeper is running. The heartbeat is wired whenever the workspace provider is active - `RELAY_WORKSPACE_ROOT` set and the provider's preflight passed; the sweeper is built only when `RELAY_WORKSPACE_MAX_AGE` or `RELAY_WORKSPACE_MIN_FREE_GB` is set, so the figure can appear with no sweeper behind it.

Note what this discloses. `GET /v1/tasks/{id}/logs` is authenticated but not admin-only and carries no per-owner gate, so any authenticated user can read any task's log, and the free-space field therefore tells them the free space in GB on a worker host's workspace volume - the same quantity `RELAY_WORKSPACE_MIN_FREE_GB` is compared against. `RELAY_SYNC_HEARTBEAT_INTERVAL=0s` does not remove it: the two bracket lines carry the same summary regardless, so the interval changes how often the figure appears, not whether it does.
...
| `tasks[].retries` | No | Retry up to this many times on failure (default `0`, max `10`). A larger or negative value is rejected at submission, and re-checked every time a stored schedule fires - see [Scheduled jobs](#scheduled-jobs), because that makes the bound retroactive over schedules created by earlier releases. It bounds one per-task multiplier, and the counts it multiplies are bounded separately. **The count caps bound ONE request; `RELAY_JOB_SUBMIT_RATE_LIMIT` bounds how often one principal may repeat it** (default `120:10s`, per authenticated user, shared with `POST /v1/jobs/{id}/retry`). Read the product plainly: 25,000 commands x 11 attempts x 120 submissions is about 33 million spawns per ten-second window at the defaults, so these caps are a blast-radius bound and not a DoS control. There is no backoff between a failed task and its redispatch, so a deterministically-failing command burns the whole budget in seconds; for a contended resource use a reservation rather than a large retry count. |
...
**`?q=` cost.** Substring containment cannot be index-served, so a `?q=` request walks every candidate row of the active sort's index and, for the count, joins `users` to reach the submitter email. A needle that matches nothing is the worst case: it pays the full walk and returns an empty page. Two server-side controls bound it, and neither makes the scan cheaper. `RELAY_JOB_SEARCH_RATE_LIMIT` bounds how often one principal may ask, charged inside the handler only when a needle is actually present. `RELAY_DB_STATEMENT_TIMEOUT` is a floor under a statement that runs away; at the cost measured below it does not bind, and it is there for the case nobody predicted. Debouncing client-side reduces how many of these a typing user generates; it never bounded what a caller can request, which is why the two controls above exist. Two properties of the cost are worth knowing before tuning either control. **The worst case is the needle that matches nothing**, not the one that matches a lot: a matching needle fills the page early and the walk stops, while a no-match needle pays the whole walk and returns empty. And **the count and the list do not scale together** - the count has no `LIMIT` and joins `users`, so it walks regardless. `docs/retros/2026-09-04-q-cost-measurement.md` records a reproducible measurement with its inputs, and `scripts/qcost` regenerates it.
...
- `PUT /v1/users/me/password` is rate-limited per authenticated user by
  ...
  bcrypt compare on every request that gets past its length guard. Over the ceiling it
  answers `429` with `Retry-After`; the caller's session is untouched.
- **The bucket is keyed on the user, so all of that user's sessions share it**, and a request
  refused for a short `new_password` is charged like any other. Somebody holding a stolen
  token can therefore keep the bucket full and hold this remedy shut. If you believe a session
  is compromised, call `DELETE /v1/auth/tokens` first - it is not bucketed, and it revokes the
  attacker's token along with your own - and change the password after.
