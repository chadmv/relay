...
On first boot the agent requires a one-time enrollment token. After successful enrollment the long-lived token is persisted and used automatically on subsequent starts. If the token is revoked by an admin, the agent exits with an authentication error **the next time it connects**. Revocation does not reach a connection that is already established: nothing re-checks a credential after registration and revoking does not close the stream, so an already-connected agent keeps receiving dispatches and keeps writing task logs and statuses until it disconnects for some other reason. Relay sets no maximum connection age, so there is no timer that ends it either. To make revocation effective now, disable or delete the worker and confirm it has gone offline.
...
Revoke the long-lived authentication token for a worker (admin only). The agent will not reconnect until re-enrolled. This takes effect at the agent's **next connection**, not on a live one - an already-connected agent keeps its stream and keeps working until it disconnects for some other reason, because nothing re-checks the credential after registration and relay sets no maximum connection age.
