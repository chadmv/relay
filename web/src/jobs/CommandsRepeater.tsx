...
interface CommandsRepeaterProps {
  taskId: string
  commands: TaskRow['commands']
  multiCommand: boolean
  onChange: (commands: TaskRow['commands'], multiCommand: boolean) => void
...
// function a test can call or spy on directly. taskId/commands/multiCommand
// are narrowed props, not the whole task, so an edit to a sibling field (name,
// timeout, retries, env, requires) leaves this row's OWN props unchanged.
export function CommandsRepeater({ taskId, commands, multiCommand, onChange, announce }: CommandsRepeaterProps) {
  const focusAfterUpdate = useFocusAfterUpdate()
  const addCommandId = `task-${taskId}-add-command`

  function setCommands(next: TaskRow['commands'], nextMultiCommand = multiCommand) {
    onChange(next, nextMultiCommand)
  }
  ...
  function addCommand() {
    ...
    setCommands([...commands, cmd], true)
    announce(`Command ${commands.length + 1} added`)
    focusAfterUpdate(`task-${taskId}-cmd-${cmd.id}-arg-${cmd.tokens[0].id}`)
  }

  function removeCommand(ci: number) {
    const gone = commands[ci]
    setCommands(commands.filter((c) => c.id !== gone.id))
    announce(`Command ${ci + 1} removed`)
    const next = commands[ci + 1] ?? commands[ci - 1]
    focusAfterUpdate(next === undefined ? addCommandId : `task-${taskId}-cmd-${next.id}-remove`)
  }

  function addToken(ci: number) {
    const token = newTokenRow()
    const cmd = commands[ci]
    setCommands(commands.map((c, j) => (j === ci ? { ...c, tokens: [...c.tokens, token] } : c)))
    announce(`Argument ${cmd.tokens.length + 1} added`)
    focusAfterUpdate(`task-${taskId}-cmd-${cmd.id}-arg-${token.id}`)
  }

  function removeToken(ci: number, ti: number) {
    const cmd = commands[ci]
    const gone = cmd.tokens[ti]
    setCommands(commands.map((c, j) => (j === ci ? { ...c, tokens: c.tokens.filter((t) => t.id !== gone.id) } : c)))
    announce(`Argument ${ti + 1} removed`)
    const next = cmd.tokens[ti + 1] ?? cmd.tokens[ti - 1]
    focusAfterUpdate(
      next === undefined
        ? `task-${taskId}-cmd-${cmd.id}-add-arg`
        : `task-${taskId}-cmd-${cmd.id}-arg-${next.id}-remove`,
    )
  }

  function editToken(ci: number, ti: number, text: string) {
    setCommands(
      commands.map((c, j) => (j === ci ? { ...c, tokens: c.tokens.map((t, k) => (k === ti ? { ...t, text } : t)) } : c)),
    )
  }
        ...
        >
          {cmd.tokens.map((token, ti) => (
            <div key={token.id} className="flex flex-wrap items-center gap-1.5">
              <Input
                id={`task-${taskId}-cmd-${cmd.id}-arg-${token.id}`}
              ...
              />
              <PillButton
                id={`task-${taskId}-cmd-${cmd.id}-arg-${token.id}-remove`}
          ...
          <div className="flex flex-wrap gap-1.5">
            <PillButton id={`task-${taskId}-cmd-${cmd.id}-add-arg`} onClick={() => addToken(ci)}>
              Add argument
            </PillButton>
            {commands.length > 1 ? (
              <PillButton
                id={`task-${taskId}-cmd-${cmd.id}-remove`}
