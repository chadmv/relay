...
test('remove does NOT refetch the deleted id\'s own detail or runs queries (Invariant 1: end the generation before releasing the resource)', async () => {
  let detailCalls = 0
  let runsCalls = 0
  server.use(
    http.get('/v1/scheduled-jobs/s1', () => {
    ...
    http.delete('/v1/scheduled-jobs/s1', () => new HttpResponse(null, { status: 204 })),
  )
  const client = new QueryClient({ defaultOptions: { queries: { retry: false } } })
  const wrapper = makeWrapper(client)

  // Both the detail and runs queries need an ACTIVE OBSERVER (mounted via renderHook,
  // not a cache seed) so that a broad invalidateQueries(['schedules']) would actually
  // try to refetch them - otherwise this test would pass vacuously regardless of
  // whether the fix works. Long intervals so the only possible extra call is the one
  // under test.
  const { result: detail } = renderHook(() => useSchedule('s1', 600_000), { wrapper })
  await waitFor(() => expect(detail.current.status).toBe('success'))
  const { result: runs } = renderHook(() => useScheduleRuns('s1', 600_000), { wrapper })
  await waitFor(() => expect(runs.current.status).toBe('success'))

  const detailCallsAfterLoad = detailCalls
  const runsCallsAfterLoad = runsCalls

  const { result: actions } = renderHook(() => useScheduleActions(), { wrapper })
  await actions.current.remove.mutateAsync('s1')

  // Give any wrongly-triggered refetch a chance to land before asserting its absence.
  await new Promise((r) => setTimeout(r, 50))
  expect(detailCalls).toBe(detailCallsAfterLoad)
  expect(runsCalls).toBe(runsCallsAfterLoad)
})
