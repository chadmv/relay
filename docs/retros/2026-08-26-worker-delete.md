...
date: 2026-08-26
topic: worker-delete
slice: worker-delete (one admin-only route, one CLI arm, five new SQL statements, one prose sweep)
branch: claude/pr-merging-session-868949
range: origin/main..HEAD (backend only; Go + SQL + `make generate`; no migration, no proto, zero files under `web/`)
pr: worker-delete - reference this work by date and slug, never by a predicted number
closes: bug-2026-08-25-no-worker-delete-at-any-layer
amends: idea-2026-08-25-ttl-reaper-for-never-reconnected-workers (unblocks its delete arm)
filed-this-slice: idea-2026-08-26-cli-has-no-integration-coverage, bug-2026-08-26-integration-lane-times-out-on-docker-teardown, bug-2026-08-26-deleting-a-worker-frees-its-hostname-to-a-race, bug-2026-08-26-re-enrolling-a-deleted-worker-escapes-its-reservations, idea-2026-08-26-should-worker-subcommands-resolve-revoked-hostnames
---

# Session Retro: 2026-08-26 - every layer refuted the layer above it, the most valuable refutation was of the conductor's own backlog item, and the headline defect is a review finding that reached the right suspicion and the wrong conclusion

**TL;DR:** `DELETE /v1/workers/{id}` (admin-only) destroys a worker identity in one transaction whose
**statement order is the correctness argument**: lock the row, requeue its live assignments while
`worker_id` still names them, unlink `agent_enrollments.consumed_by`, scrub `reservations.worker_ids`,
count the terminal tasks about to lose attribution, then `DELETE` behind a
`status IN ('offline','revoked')` allow-list. `relay workers delete --yes <id-or-hostname>` is the CLI
arm, and the ghost-command guard that has held this gap open since 2026-08-25 now goes green **because
the command exists**.
...
- **The five filed items** (frontmatter): CLI integration coverage; the Docker teardown timeout; the
