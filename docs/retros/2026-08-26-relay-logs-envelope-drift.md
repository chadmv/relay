...
- [`bug-2026-08-26-command-constructors-writer-wiring-is-unpinned`](../backlog/bug-2026-08-26-command-constructors-writer-wiring-is-unpinned.md) - nothing constructs `LogsCommand()` or `SubmitCommand()`, so the `os.Stdout, os.Stderr` pair transposes silently. `captureStdStreams` already exists in the package.
