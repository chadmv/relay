...
| `PATCH` that changes `job_spec`, `cron_expr` or `timezone` | **CLEARED** | The handler validated the new values before storing them, so any recorded failure about the old ones is stale by construction. These are exactly the three inputs the three recorded classes are about. |
