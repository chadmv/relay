...
- [ ] **A8.** **Human step, not automatable from the repo.** `cli integration (real server)` is added to the required status checks for `main`. Until then the job is advisory. If declined, take Task 10 Step 6's fallback.
