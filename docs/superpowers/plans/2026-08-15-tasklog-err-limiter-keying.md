...
| M11 | **Both handlers share one bad-id key** | Add `kindBadTaskIDStatus` to the `logKind` block and use it at `handleTaskStatus`'s parse guard only | I | `TestHandleTaskLog_MalformedTaskIdIsLoggedOncePerConnectionAndSharesTheStatusBudget` LEG B | `assert.Equal(t, 1, total, "both handlers share ONE bad-task-id budget key")` - got **2** |
