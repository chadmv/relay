...
The load-bearing evidence is PRECEDENT, not semver theory. Commit `e536f3e` (#156) added a REQUIRED `seq` to the already-exported `LogRecord` - a constructor-breaking change to a response-only model, the same shape as removing `Page`'s defaults - and went `0.1.2 -> 0.2.0`, a MINOR bump. Its own commit message says why: "`LogRecord.seq` became REQUIRED, which is backwards-incompatible, and a patch bump advertises no breakage." Name `LogRecord`, not `LogPage`, and the distinction IS the precedent rather than pedantry: `git show e536f3e^:python/src/relay/models.py` has no `class LogPage` at all, because that commit INTRODUCED it with its fields required from birth - which breaks nothing and is therefore no precedent for a minor bump on a breaking change. An earlier pass corrected this sentence's QUOTE while leaving its SUBJECT wrong, which is the correcting-a-claim-regenerates-it shape. This slice is that exact shape on the sibling envelope and took a patch. (`69cef30`/#161 took a patch, but it only ADDED raise sites; it did not change a public model's construction signature, so it is not the counter-example it looks like.)
