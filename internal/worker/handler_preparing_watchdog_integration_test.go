...
//
// IT HAS NO RED-FIRST RUN, because a preparing row is not representable before
// this slice. Its RED is supplied by a named mutation - handler.go's
// `if statusStr == "running"` widened to `|| statusStr == "preparing"`, which is
// the regression this design exists to avoid - and that mutation must be run and
// recorded, or this test is a shape and not a proof.
//
